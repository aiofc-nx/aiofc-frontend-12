...
```bash
pnpm create remix@latest --init-script --install --template https://github.com/PhilDL/aiofc-frontend-1229
```
...
```bash
git clone git@github.com:PhilDL/aiofc-frontend-1229.git
cd aiofc-frontend-1229
...
- `apps` Folder containing the applications
  - [`remix-app`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/apps/remix-app): the [Remix.run](https://remix.run) app in ESM.
  - [`remix-vercel`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/apps/remix-vercel): the [Remix.run](https://remix.run) app, ready to be deployed on [Vercel](https://vercel.com).
  - [`nextjs-app`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/apps/nextjs-app): a [Next.js](https://nextjs.org) app
- `packages` Folder containing examples

  - [`ui`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/packages/ui): a React UI package example powered by [shadcn/ui](https://ui.shadcn.com/). Some example components and shadcn/ui Tailwind config exported as Tailwind plugin and preset.
  - [`database`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/packages/database): a [Prisma](https://prisma.io) wrapper ready to be used in other packages, or apps. Bundled with [tsup](https://tsup.egoist.dev/). Can be PostgreSQL or SQLite // Litefs dependening of what you choose during installation.
  - [`business`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/packages/business): an example package using the Prisma `database` as a dependency and using a _repository pattern_ like example.
  - [`internal-nobuild`](https://github.com/PhilDL/aiofc-frontend-1229/tree/main/packages/internal-nobuild): an example package that is pure TypeScript with no build steps. The `main` entrypoint to the package is directly `src/index.ts`. Remix takes care of compiling with its own build step (with esbuild). This packages also contains unit test with Vitest.
...
```bash
turbo gen workspace --name @aiofc/foobarbaz --type package --copy
```
...
- Run the Cypress tests and Dev
  ```bash
  pnpm run test:e2e:dev --filter=@aiofc/remix-app
  ```
- Lint everything
  ```bash
  pnpm run lint
...
- How to install an npm package in the Remix app ?
  ```bash
  pnpm add dayjs --filter @aiofc/remix-app
  ```
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create aiofc-frontend-1229
  fly apps create aiofc-frontend-1229-staging
  ```
...
```sh
fly postgres create --name aiofc-frontend-1229-db
fly postgres attach --app aiofc-frontend-1229 aiofc-frontend-1229-db

fly postgres create --name aiofc-frontend-1229-staging-db
fly postgres attach --app aiofc-frontend-1229-staging aiofc-frontend-1229-staging-db
```
...
```sh
fly volumes create data --region cdg --size 1 --app aiofc-frontend-1229
fly volumes create data --region cdg --size 1 --app aiofc-frontend-1229-staging
```

Then attach the volumes to the apps:

```sh
fly consul attach --app aiofc-frontend-1229
fly consul attach --app aiofc-frontend-1229-staging
```
...
If you found the template useful, please consider giving it a [Star ⭐](https://github.com/PhilDL/aiofc-frontend-1229). Thanks you!
